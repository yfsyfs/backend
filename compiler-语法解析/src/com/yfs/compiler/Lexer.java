...
public class Lexer {
	...
	public static final int EOF = 0; // 文件末尾
	...
	public int lookAhead = -1; // 等于上面的枚举值之一,表示当前的text到底是什么, 对应编译原理里面的lookAhead技巧
	public String current = ""; // 当前用户输入的字符串（inputBuffer被截的七零八落的剩余结果就是current）
	public String inputBuffer = ""; // 仅仅用于存储用户的输入（end之前的所有内容）
	public int length; // text的长度
	public String text; // 文章中所谓的"有意义的"字符串, 编译原理里面叫symbol（符号）
	...
	public void advance() {
		lookAhead = lex(); // 单纯往后读取一个字符,并返回它的类型
	}

	public boolean match(int token) {
		if (lookAhead == -1) { // 如果当前字符是 -1，即是刚开始, 则就要往后读取一个字符
			lookAhead = lex();
		}
		return token == lookAhead;
	}

	public int lex() { // 读取下一个字符
		while (true) {
