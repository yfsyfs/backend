...
public class DeviceLdapServiceImpl implements DeviceLdapService {
	...
	@Override
	public Page<Device> queryBlacklistDeviceByConditions(Map<String, Object> params, Page<Device> page)
			throws IOException {
		int size = page.getSize();
		int start = (page.getCur() - 1) * size + 1;
		int end = page.getCur() * size;
		// 设备名称
		final String cn = params != null && !params.isEmpty() ? (String) params.get("cn") : null;
		final SearchControls searchControls = new SearchControls();
		searchControls.setSearchScope(SearchControls.SUBTREE_SCOPE);
		final PagedResultsDirContextProcessor processor = new PagedResultsDirContextProcessor(size);
		List<Device> devices = SingleContextSource.doWithSingleContext(ldapTemplate.getContextSource(),
				new LdapOperationsCallback<List<Device>>() {
					int count = 0;

					@Override
					public List<Device> doWithLdapOperations(LdapOperations operations) {
						List<Device> ret = new ArrayList<>();
						do {
							// 向ldap服务器一次取回的数据
							List<Device> result = operations.search(Constants.DEVICE_BASE_DN,
									cn == null ? "resourceType=acsDevice"
											: ("(&(cn=*" + cn + "*)(resourceType=acsDevice))"),
									searchControls, new DeviceLdapMapper(), processor);
							boolean flag = true;
							for (Device device : result) {
								Device d = deviceMapper.findOneByResourceId(device.getResourceId());
								if (d != null && "on".equals(d.getStatus())) {
									count++;
